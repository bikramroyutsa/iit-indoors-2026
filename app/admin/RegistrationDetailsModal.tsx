  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm">
      <div className="bg-deep-teal border-8 border-black shadow-[0_0_0_4px_var(--mint),_12px_12px_0px_#000] p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto custom-scrollbar relative">
        <button onClick={() => setSelectedReg(null)} className="absolute top-4 right-4 text-mint hover:text-white text-2xl">×</button>
        ...
        <div className="flex gap-4 mt-8 pt-4 border-t-2 border-mint-soft">
          {selectedReg.status !== "accepted" && (
            <button onClick={() => handleUpdateStatus(selectedReg.id, "accepted")} className="pixel-button w-full bg-mint text-deep-teal hover:bg-mint-soft">accept</button>
          )}
          {selectedReg.status !== "rejected" && (
            <button onClick={() => handleUpdateStatus(selectedReg.id, "rejected")} className="pixel-button w-full !bg-red-500 !border-black !text-white hover:!bg-red-400" style={{ boxShadow: '0 -6px 0 -2px #f87171, 0 6px 0 -2px #f87171, -6px 0 0 -2px #f87171, 6px 0 0 -2px #f87171, 6px 6px 0px #000' }}>reject</button>
          )}
        </div>
      </div>
    </div>
